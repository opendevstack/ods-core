...
b) inside the [/tests](tests) directory. </p>

The tests can be started with `make test`, which will call two test-suites. Namely, the tests to create a new project in [/tests/create-projects](tests/create-projects) and those in [/tests/ods-verify](tests/ods-verify). If both pass - the setup of ods-core components is successfull. <br>Running the ods-core tests is also a pre-condition for the [ods-quickstarter tests](https://github.com/opendevstack/ods-quickstarters/tree/master/tests), as the test projects are created thru the tests here.
