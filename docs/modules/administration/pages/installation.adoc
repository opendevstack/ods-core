...
In the course of this you have to set a new admin password.
...
The script will prompt for a new admin password, once set, an auth token that is used by the Jenkins pipelines will be generated.
Both values are automatically written into `ods-configuration/ods-core.env`, which you then need to commit and push after adding the Bitbucket `ods-configuration` repo as the remote origin.
