...
On your own Bitbucket instance, a new `OPENDEVSTACK` project will be created with the necessary repositories, such as `ods-core`. To set them up, use:
...
Notice that `prepare-bitbucket-repos` expects a running Atlassian suite. It will then create the OPENDEVSTACK project and the repos inside. It will *not* spin up a Bitbucket instance. You'll need to setup Bitbucket prior to running this. You can make use of either the Ansible playbooks in infrastructure-setup (however beware that this hasn't been updated in a while and might be buggy) or take inspiration from the ods-devenv setup, which uses containers to run Bitbucket.
