...
#    curl -O https://repo1.maven.org/maven2/mysql/mysql-connector-java/8.0.20/mysql-connector-java-8.0.20.jar
    ...
    docker run -v crowdVolume:/var/atlassian/application-data/crowd --name="crowd" -d -p ${atlassian_crowd_port}:${atlassian_crowd_port_internal} atlassian/crowd:${atlassian_crowd_software_version}

    docker cp crowd-backup.xml crowd:/var/atlassian/application-data/crowd/shared/backups/crowd-backup.xml
    docker exec -it crowd bash -c 'chmod 444 /var/atlassian/application-data/crowd/shared/backups/crowd-backup.xml; ls -lart /var/atlassian/application-data/crowd/shared/backups/'
...
-b crowd_sessionid_cookie.txt \
--form 'filePath=/var/atlassian/application-data/crowd/shared/crowd-backup.xml'

    # Test setup by login into crowd
curl --location --request POST 'http://localhost:8095/crowd/console/login.action' \
--header 'Content-Type: text/plain' \
--header 'Cookie: JSESSIONID=E8E719E31F6A5C6010409B7459293EE2' \
...
#        -e ATL_DB_TYPE=mysql \
#        -e ATL_DB_SCHEMA_NAME= \
#        atlassian/jira-software:${atlassian_crowd_software_version}
#    local crowd_ip=$(docker inspect --format '{{.NetworkSettings.IPAddress}}' ${atlassian_crowd_container_name})
    # this race condition normally works out. Did not cause any trouble yet.
    # Alternative approach: start container, stop container, cp driver, restart container ...
#    docker container cp mysql-connector-java-8.0.20.jar jira:/opt/atlassian/jira/lib/
