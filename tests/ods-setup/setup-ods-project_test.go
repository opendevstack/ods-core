...
func TestCreateOdsProject(t *testing.T) {
    ...
    if err = utils.FindRoleBinding(roleBindings, "system:authenticated", "Group", "", "system:image-puller"); err != nil {
        t.Fatal(err)
    }
    ...
    if err = utils.FindClusterRoleBinding(clusterRoleBindings, fmt.Sprintf("system:serviceaccount:%s:deployment", namespace), "ServiceAccount", namespace, "cluster-admin"); err != nil {
        t.Fatal(err)
    }
